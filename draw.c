...
#define COLOR(r, g, b) ((color_old){ (r) / ((double)0xff), (g) / ((double)0xff), (b) / ((double)0xff) })
#define CHANNEL(i) ((i) / ((double)0xff))
...
color_old white = COLOR(0xff, 0xff, 0xff);
color_old black = COLOR(0x00, 0x00, 0x00);
color_old darkbrown = COLOR(0xd1, 0x8b, 0x47);
color_old lightbrown = COLOR(0xff, 0xce, 0x9e);

//cairo_pattern_t *white;
//cairo_pattern_t *black;
//cairo_pattern_t *darkbrown;
//cairo_pattern_t *lightbrown;

cairo_t *cr;

static void draw_board(double x, double y, double size)
{
	cairo_push_group(cr);
	cairo_set_antialias(cr, CAIRO_ANTIALIAS_NONE);

	double fieldsize = size / NUM_ROW_FIELDS;
	for (int j = 0; j < NUM_ROW_FIELDS; ++j) {
		for (int i = 0; i < NUM_ROW_FIELDS; ++i) {
			if ((i + j) % 2 == 0) {
				cairo_set_source_rgb(cr, lightbrown.r, lightbrown.g, lightbrown.b);
			} else {
				cairo_set_source_rgb(cr, darkbrown.r, darkbrown.g, darkbrown.b);
			}
			double fx = x + i * fieldsize;
			double fy = y + j * fieldsize;
			cairo_rectangle(cr, fx, fy, fieldsize, fieldsize);
			cairo_fill(cr);
		}
	}
...
static void draw_king(double x, double y, double size, color c)
{
	color_old fill = c == COLOR_WHITE ? white : black;
	color_old stroke = c == COLOR_WHITE ? black : white;
	...
	cairo_set_source_rgb(cr, stroke.r, stroke.g, stroke.b);
	cairo_stroke(cr);
...
static void draw_queen(double x, double y, double size, color c)
{
	color_old fill = c == COLOR_WHITE ? white : black;
	color_old stroke = c == COLOR_WHITE ? black : white;
...
static void draw_rook(double x, double y, double size, color c)
{
	color_old fill = c == COLOR_WHITE ? white : black;
	color_old stroke = c == COLOR_WHITE ? black : white;
...
static void draw_bishop(double x, double y, double size, color c)
{
	color_old fill = c == COLOR_WHITE ? white : black;
	color_old stroke = c == COLOR_WHITE ? black : white;
...
static void draw_knight(double x, double y, double size, color c)
{
	color_old fill = c == COLOR_WHITE ? white : black;
	color_old stroke = c == COLOR_WHITE ? black : white;
...
static void draw_pawn(double x, double y, double size, color c)
{
	color_old fill = c == COLOR_WHITE ? white : black;
...
void draw_piece(int piece, double x, double y, double size)
{
	switch (piece) {
		...
		case PIECE_WHITE_KING:
			draw_king(x, y, size, COLOR_WHITE);
			return;
		case PIECE_BLACK_KING:
			draw_king(x, y, size, COLOR_BLACK);
			return;
		case PIECE_WHITE_QUEEN:
			draw_queen(x, y, size, COLOR_WHITE);
			return;
		case PIECE_BLACK_QUEEN:
			draw_queen(x, y, size, COLOR_BLACK);
			return;
		case PIECE_WHITE_ROOK:
			draw_rook(x, y, size, COLOR_WHITE);
			return;
		case PIECE_BLACK_ROOK:
			draw_rook(x, y, size, COLOR_BLACK);
			return;
		case PIECE_WHITE_BISHOP:
			draw_bishop(x, y, size, COLOR_WHITE);
			return;
		case PIECE_BLACK_BISHOP:
			draw_bishop(x, y, size, COLOR_BLACK);
			return;
		case PIECE_WHITE_KNIGHT:
			draw_knight(x, y, size, COLOR_WHITE);
			return;
		case PIECE_BLACK_KNIGHT:
			draw_knight(x, y, size, COLOR_BLACK);
			return;
		case PIECE_WHITE_PAWN:
			draw_pawn(x, y, size, COLOR_WHITE);
			return;
		case PIECE_BLACK_PAWN:
			draw_pawn(x, y, size, COLOR_BLACK);
			return;
		default:
			BUG();
	}
...
void draw_game(int board[NUM_ROW_FIELDS][NUM_ROW_FIELDS], double x, double y, double size)
{
	...
	for (int j = 0; j < NUM_ROW_FIELDS; ++j) {
		for (int i = 0; i < NUM_ROW_FIELDS; ++i) {
			draw_piece(board[i][j], x + i * fieldsize, y + j * fieldsize, fieldsize);
		}
	}
}
